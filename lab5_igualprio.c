...
short int leit = 0,escr = 0,l_fila = 0,e_fila = 0,inicio = 1,ultimo_e = 0;
...
void ini_leit(int id)
{
	...
	l_fila++;
	while((inicio || escr > 0) || (e_fila > 0 && !ultimo_e))//bloqueia se houverem escritores na fila e um leitor foi o ultimo a ler
	{
		
		printf("Leitor %d bloqueou-se,%d leitores na fila e %d escritores na fila\n",id, l_fila,e_fila);
...
void ini_escr(int id)
{
	...
	while(escr >0|| leit >0 || (l_fila > 0 && ultimo_e ))//espera outros escritores
	{
